...
class OmciPayoutProcessor:
    ...
    def process(self):
        ...
        correspondance_idoperator = dfop_unique.set_index(match_key_partner)['idop']
        ...
        service['Taux(%)']=-(service['frais_omci'] / service['Debit'])
        select_taux=service.groupby(['Paiement']).agg(
                    Taux=('Taux(%)', 'mean')
                )
        
        #NBSI PMT &CINETPAY
        dfpmt['OMCI'] = dfpmt[match_key_pmt].isin(service[match_key_partner]).astype(int)
