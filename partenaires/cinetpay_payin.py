...
class CinetpayPayinProcessor:
    def __init__(self, data_file, partner_file, reco_start=None, reco_end=None,
        ...
        self._partner_label = "CINETPAY PAYIN"
    ...
    def process(self):
        ...
        match_key_partner = self.match_col_partner or 'ID transaction'
        ...
        correspondance_statut_refused= refused.set_index(match_key_partner)['Statut']
        correspondance_date_op = dfop.set_index(match_key_partner)['Date']
        correspondance_id_op = dfop.set_index(match_key_partner)['ID Operator']
        correspondance_statut_pmt = dfpmt.set_index('Transaction ID')['Statut']
        correspondance_commentaire = refused.set_index(match_key_partner)['Commentaire']
        correspondance_operator = dfop.set_index(match_key_partner)['Opérateur']
        
        dfpmt['DATE_OP'] = dfpmt[match_key_pmt].map(correspondance_date_op)
        refused['STATUT_PMT'] = refused['ID transaction'].map(correspondance_statut_pmt)
        dfpmt['STATUT_OP'] = dfpmt[match_key_pmt].map(correspondance_statut_op)
        dfpmt['REFUSED'] = dfpmt[match_key_pmt].map(correspondance_statut_refused)
        dfpmt['ID_OP'] = dfpmt[match_key_pmt].map(correspondance_id_op)
        dfpmt['Comment'] = dfpmt[match_key_pmt].map(correspondance_commentaire)
        dfpmt['OPERATOROP'] = dfpmt[match_key_pmt].map(correspondance_operator)
        ...
        #NBSI PMT &CINETPAY
        dfpmt['CINETPAY'] = dfpmt[match_key_pmt].isin(dfcinetpay[match_key_partner]).astype(int)
        dfcinetpay['PMT'] = dfcinetpay[match_key_partner].isin(dfpmt[match_key_pmt]).astype(int)
        dfpmt['REFUSED'] = dfpmt[match_key_pmt].isin(refused[match_key_partner]).astype(int)
...
# ================================
        ...
        with tabs[1]:
            ...
            try:
                matched_part = int(dfcinetpay['PMT'].sum())
            except Exception:
                matched_part = int((dfcinetpay['PMT'] == 1).sum()) if hasattr(dfcinetpay, '__len__') else 0
