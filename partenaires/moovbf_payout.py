...
class MoovbfPayoutProcessor:
    def __init__(self, data_file, partner_file, reco_start=None, reco_end=None,
        ...
        self._partner_label = "MOOV BF PAYOUT"
    ...
    def process(self):
        ...
        payin = dfop[(dfop['Transaction Status'] == 'Completed') & (dfop['Details'] == 'Online Merchant Transfer')]

        payin["Withdrawn"] = pd.to_numeric(payin["Withdrawn"], errors="coerce")
        payin["Fee"] = pd.to_numeric(payin["Fee"], errors="coerce")
        payin['Taux(%)'] = (payin["Fee"] / payin["Withdrawn"]) * 100
        ...
        dfop_unique['Nombre'] = dfop_unique['Withdrawn']
...
# ================================
        ...
        with tabs[1]:
            st.subheader("Rapport Reconciliation Moov BF Payout")
        ...
        # Création du tableau croisé dynamique
            ...
            maj_failed_a_succes = dfpmt.loc[(dfpmt['Statut'] != 'FAILED') & (dfpmt['MOOV'] == 1)]
