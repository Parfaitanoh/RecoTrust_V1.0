...
class BizaoPayinProcessor:
    ...
    def process(self):
        ...
        match_key_pmt = self.match_col_pmt or 'External Transaction Id'
        match_key_partner = self.match_col_partner or 'Order ID'
        # traitement  pour PMT
        ...
        correspondance_statut_op = dfop_unique.set_index(match_key_partner)['Status']
        correspondance_date_op = dfop_unique.set_index(match_key_partner)['Date'].astype(object)
        correspondance_idoperator = dfop_unique.set_index(match_key_partner)['Transaction ID']
        correspondance_operator = dfop_unique.set_index(match_key_partner)['operator']
        ...
        dfpmt['OPERATORBIZAO'] = dfpmt[match_key_pmt].map(correspondance_operator)
        ...
        #NBSI PMT &CINETPAY
        dfpmt['BIZAO'] = dfpmt[match_key_pmt].isin(ts[match_key_partner]).astype(int)
