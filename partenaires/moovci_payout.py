...
class MoovciPayoutProcessor:
    def __init__(self, data_file, partner_file, reco_start=None, reco_end=None,
        ...
        self._partner_label = "MOOV CI PAYOUT"
    ...
    def process(self):
        ...
        match_key_partner = self.match_col_partner or 'Receipt No.'
        ...
        dfop = dfop.drop_duplicates(subset=match_key_partner)
        
        # Vérification des correspondances entre A1 et B1
        correspondance_statut_op = dfop.set_index(match_key_partner)['Transaction Status']
        correspondance_date_op = dfop.set_index(match_key_partner)['Date'].astype(object)
        correspondance_idoperator = dfop.set_index(match_key_partner)['idop']
        ...
        #NBSI PMT &CINETPAY
        dfpmt['MOOVCI'] = dfpmt[match_key_pmt].isin(payin[match_key_partner]).astype(int)
        payin['PMT'] = payin[match_key_partner].isin(dfpmt[match_key_pmt]).astype(int)

        dfpmt['Nombre']= dfpmt['Montant']
        payin['Nombre']= payin['Withdrawn']
...
# ================================
        ...
        with tabs[1]:
            st.subheader("Rapport Reconciliation MOOV CI PAYOUT")
        ...
        # Création du tableau croisé dynamique
            ...
            df_filtered,
            values=['Nombre', 'Withdrawn'],
            index=['Date','Transaction Status'],
            aggfunc={'Nombre': 'count','Withdrawn': 'sum' },
            fill_value=0,
            margins=True,
            margins_name='Total'
        )
            ...
            maj_failed_a_succes = dfpmt.loc[(dfpmt['Statut'] == 'PENDING') & (dfpmt['MOOVCI'] == 1)]
