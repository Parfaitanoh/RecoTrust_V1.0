...
class OrangesnPayinProcessor:
    def __init__(self, data_file, partner_file, reco_start=None, reco_end=None,
        ...
        self._partner_label = "ORANGE SN PAYIN"
    ...
    def process(self):
        ...
        match_key_partner = self.match_col_partner or 'Référence Partenaire'
        ...
        # Filtre métier Orange SN (préservé)
        if 'Services' in dfop.columns and 'Statut' in dfop.columns:
            service = dfop.loc[
                (dfop['Services'].astype(str).str.strip() == 'Paiement marchand OTP')
                & (dfop['Statut'].astype(str).str.strip() == 'SUCCESS')
            ].copy()
            if service.empty:
                service = dfop.copy()
        else:
        ...
        # Date courte à partir de "Date & Heure" si présent
        if 'Date & Heure' in service.columns:
            def _extracted(x):
                try:
                    return str(x).split(' ')[0]
                except Exception:
                    return x
            service['Date'] = service['Date & Heure'].apply(_extracted)
        elif 'Date' not in service.columns:
            service['Date'] = pd.NaT

        # --- Résolution défensive des colonnes de matching (custom UI) ---
        def _resolve_col(df, wanted, label):
            ...
            cols_preview = ', '.join(map(str, list(df.columns)[:25]))
            more = '…' if len(df.columns) > 25 else ''
            raise KeyError(
                f"Colonne de matching {label} '{wanted}' introuvable. "
                f"Colonnes disponibles : {cols_preview}{more}"
            )

        match_key_pmt = _resolve_col(dfpmt, match_key_pmt, 'PMT')
        match_key_partner = _resolve_col(service, match_key_partner, 'Partenaire')

        # Colonne d'identifiant opérateur pour enrichissement (fallback = clé de match)
        _idop_col = None
        for _cand in ('Référence OM', 'Reference OM', 'Référence Partenaire', 'Reference', match_key_partner):
            if _cand and _cand in service.columns:
                _idop_col = _cand
                break
        if _idop_col is None:
            _idop_col = match_key_partner

        if 'Statut' not in service.columns:
            service['Statut'] = 'UNKNOWN'

        #MISE EN PLACE DE RECHERCHE X POUR RECUPERATION CHEZ LE PARTENAIRE
        dfop_unique = service.drop_duplicates(subset=match_key_partner)
        ...
        correspondance_idoperator = dfop_unique.set_index(match_key_partner)[_idop_col]
        ...
        #select_taux=service.groupby(['Paiement']).agg(
            #Taux=('Taux(%)', 'mean')
        #)
        #NBSI PMT &CINETPAY
        dfpmt['OMSN'] = dfpmt[match_key_pmt].isin(service[match_key_partner]).astype(int)
        service['PMT'] = service[match_key_partner].isin(dfpmt[match_key_pmt]).astype(int)

        dfpmt['Nombre']= dfpmt['Montant']
        service['Nombre']= service['Montant']
...
# ================================
        ...
        with tabs[1]:
            st.subheader("Rapport Reconciliation ORANGE SN PAYIN")
            
            # Création du tableau croisé dynamique
            df_filteredpmt = dfpmt[dfpmt['OMSN'] == 1]
            
            matched = df_filteredpmt['OMSN'].sum()
            unmatched = len(dfpmt) - matched
            try:
                matched_part = int(service['PMT'].sum())
            except Exception:
                matched_part = int((service['PMT'] == 1).sum()) if hasattr(service, '__len__') else 0
        ...
        # Création du tableau croisé dynamique
            ...
            maj_failed_a_succes = dfpmt.loc[(dfpmt['Statut'] != 'SUCCESS') & (dfpmt['OMSN'] == 1)]
            #maj_pending_a_succes = dfpmt.loc[(dfpmt['Statut'] == 'PENDING') & (dfpmt['OMSN'] == 1)]
            trx_succes_abs = dfpmt.loc[(dfpmt['Statut'] == 'SUCCESS') & (dfpmt['OMSN'] == 0)]
            ...
            trx_en_attente_abs= dfpmt.loc[(dfpmt['Statut']=='PENDING') & (dfpmt['OMSN'] == 0)]
