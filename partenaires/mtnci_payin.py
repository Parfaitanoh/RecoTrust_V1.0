...
class MtnciPayinProcessor:
    ...
    def process(self):
        ...
        match_key_partner = self.match_col_partner or 'Externalid'
        ...
        #NBSI PMT &CINETPAY
        dfpmt['MTNCI'] = dfpmt[match_key_pmt].isin(debit[match_key_partner]).astype(int)
        debit['PMT'] = debit[match_key_partner].isin(dfpmt[match_key_pmt]).astype(int)
...
# ================================
        ...
        with tabs[1]:
            ...
            try:
                matched_part = int(debit['PMT'].sum())
            except Exception:
                matched_part = int((debit['PMT'] == 1).sum()) if hasattr(debit, '__len__') else 0
        ...
        # Création du tableau croisé dynamique
            ...
            maj_failed_a_succes = dfpmt.loc[(dfpmt['Statut'] != 'SUCCESS') & (dfpmt['MTNCI'] == 1)]
